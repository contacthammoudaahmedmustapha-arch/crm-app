// api/users/[id].js — PUT (edit/toggle/reset) — admin only
...
export default async function handler(req, res) {
  ...
  // PUT — update user info, toggle active, or reset password
  if (req.method === 'PUT') {
    ...
    try {
      ...
      // Default: update user info
      await pool.query(
        'UPDATE users SET nom=?, prenom=?, username=?, role=? WHERE id=?',
        [nom, prenom, username, role, id]
      );
