// api/contacts/index.js — GET (list) + POST (create)
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Admin sees all, user sees all (but can only edit own)
    if (search && search.trim()) {
      const s = `%${search.trim()}%`;
      query = `SELECT * FROM contacts WHERE 
        (nom LIKE ? OR prenom LIKE ? OR societe LIKE ? OR telephone LIKE ?)
    ...
    try {
      const [result] = await pool.query(
        `INSERT INTO contacts 
          (nom, prenom, telephone, email, societe, domaine, fonction, interet, avis_visite, potentiel, created_by, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
